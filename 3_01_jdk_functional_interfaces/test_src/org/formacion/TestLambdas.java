...
public class TestLambdas {
	...
	public void test_extras() {
		...
		Predicate<Persona> predApellidoNull = p -> p.getApellido2() == null ? true : false;

		assertTrue(predApellidoNull.test(personaSinSegundoApellido));
		assertFalse(predApellidoNull.test(personaConSegundoApellido));
		assertFalse(predApellidoNull.test(personaNoPariente));

		// Una funcion que nos diga si dos personas son parientes: para nosotros
		// parientes
		// son personas con el mismo primer apellido
		BiPredicate<Persona, Persona> predSonParientes = (p1, p2) -> p1.getApellido1().equals(p2.getApellido1());

		assertTrue(predSonParientes.test(personaSinSegundoApellido, personaConSegundoApellido));
		assertFalse(predSonParientes.test(personaSinSegundoApellido, personaNoPariente));
		assertFalse(predSonParientes.test(personaNoPariente, personaConSegundoApellido));

		// Una funcion que "enmascare" los datos de una persona: debe permutar los
		// valores de sus
		// y nombre
		Consumer<Persona> masc = p -> {
		...
		masc.accept(personaConSegundoApellido);
